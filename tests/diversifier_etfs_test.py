"""Valida los diversificadores por clase de activo (ver config/universe.yaml ->
`diversifier_etfs`: VNQ/DBC/IEF -- REITs, commodities amplios, bonos de
duración intermedia): que entran al universo de descarga, que reciben su
propio tope de peso (`diversifier_etf`, ver `build_position_caps` en
...
backtest real (2015-2026) mostrara que VNQ/DBC/IEF (junto con EFA/EEM, ver
`tests/international_diversification_test.py`) diluían `avg_monthly_return`
del ensamble por debajo del objetivo -- ver el comentario en
`config/live_params.yaml -> include_satellite_etfs`.

No corre el backtest completo (eso ya lo hace `tests/integration_smoke.py`,
que lee `config/universe.yaml` de verdad y por lo tanto ya ejercita VNQ/DBC/IEF
con datos sintéticos de punta a punta) -- este test aísla solo el cableado de
universo/topes/flag, con especial atención a la clase de bug real que ya pasó
una vez con `international_etfs`: agregar el cableado a un lado (backtest o
config) y olvidarlo en el otro (ver `test_run_live_once_respects_include_satellite_etfs_flag`).
"""
import sys
import pathlib
...
def test_universe_yaml_declares_diversifier_etfs():
    universe = load_universe()
    assert "diversifier_etfs" in universe, "falta la sección diversifier_etfs en config/universe.yaml"
    div = set(universe["diversifier_etfs"])
    assert div == {"VNQ", "DBC", "IEF"}, f"se esperaba exactamente {{VNQ, DBC, IEF}}, se encontró {div}"


def test_all_tickers_includes_diversifier_etfs():
    ...
    universo de las ESTRATEGIAS, no si se descargan (descargarlos de más no
    cuesta nada y permite activar el flag sin re-descargar todo)."""
    universe = load_universe()
    tickers = all_tickers(universe)
    for t in universe["diversifier_etfs"]:
        assert t in tickers, f"{t} debería estar en all_tickers() para que se descargue su historial"


def test_position_caps_assigns_dedicated_diversifier_cap():
    universe = load_universe()
    position_caps = {"broad_etf": 0.20, "sector_etf": 0.40, "individual_stock": 0.08, "diversifier_etf": 0.15}
    caps = build_position_caps(universe, position_caps)
    for t in universe["diversifier_etfs"]:
        assert caps[t] == 0.15, f"{t} debería recibir el tope 'diversifier_etf' (0.15), recibió {caps.get(t)}"
    assert caps["VNQ"] != position_caps["broad_etf"]
...
def test_base_universe_construction_respects_include_satellite_etfs_flag():
    ...
    include_satellite_etfs es false): con el flag en false (default),
    VNQ/DBC/IEF NO deberían estar en base_universe; con el flag en true, sí --
    en ningún caso deberían estar en sector_universe."""
    universe = load_universe()
...
def test_run_live_once_respects_include_satellite_etfs_flag():
    """Regresión de la MISMA clase de bug que ya pasó una vez con
    international_etfs: run_backtest.py y run_live_once.py arman `base_universe`
    con su propia copia de la lógica (no comparten una función) -- que el flag
    apague/prenda VNQ/DBC/IEF en un lado y se le olvide en el otro dejaría al
...
def main():
    print("[1/6] Probando que universe.yaml declara VNQ/DBC/IEF...")
    test_universe_yaml_declares_diversifier_etfs()
    print("\n[2/6] Probando que all_tickers() los incluye (se descargan)...")
    test_all_tickers_includes_diversifier_etfs()
    print("\n[3/6] Probando que reciben su propio tope de peso...")
    test_position_caps_assigns_dedicated_diversifier_cap()
    ...
    print("\nDIVERSIFIER ETFS TEST OK: VNQ/DBC/IEF están correctamente cableados (backtest y en vivo, ambos casos del flag).")
