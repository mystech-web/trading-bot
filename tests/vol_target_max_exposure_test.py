...
def test_monte_carlo_generated_for_all_three_ensemble_variants():
    """Regresión de un error real: `monte_carlo.json` (y el stress test) se
    calculaban SOLO sobre el ensamble de mezcla fija -- decidir si conviene
    subir `max_gross_exposure` mirando ese Monte Carlo es mirar la variante
    equivocada, ya que `--vol-target-max-exposure` no la afecta en absoluto.
    Ahora deben generarse las 3 variantes por separado."""
    import json
    import scripts.run_backtest as rb
    _run_and_capture_max_exposure(rb, ["--vol-target-max-exposure", "1.3"])

    reports_dir = rb.resolve_profile("conservative")[1]
    mc_fixed = json.loads((reports_dir / "monte_carlo.json").read_text())
    mc_dynamic = json.loads((reports_dir / "monte_carlo_dynamic_alloc.json").read_text())
    mc_vol_target = json.loads((reports_dir / "monte_carlo_dynamic_alloc_vol_target.json").read_text())
    ...
    # No deberían ser el mismo archivo por accidente (series distintas de verdad).
    ...
    print("  las 3 variantes del Monte Carlo se generaron por separado y con contenido propio")


def main():
    print("[1/3] Probando que sin el flag se usa el max_gross_exposure de config (1.0)...")
